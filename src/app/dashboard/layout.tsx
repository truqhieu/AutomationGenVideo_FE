...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
    ...
    const menuItems = [
        { icon: Home, label: "Dashboard", href: "/dashboard", roles: ["ADMIN", "MANAGER", "EDITOR", "CONTENT"] },
        {
            ...
            roles: ["ADMIN", "MANAGER", "EDITOR", "CONTENT"],
        },
    ...
    const filteredMenuItems = menuItems.filter(
        (item) => user && item.roles.some((role) => user.roles?.includes(role as UserRole)),
    );
    ...
    return (
        <div className="min-h-screen bg-gray-50">
            ...
            <div
                className={`transition-[padding-left] duration-200 ease-out ${sidebarOpen ? "pl-[320px]" : "pl-[50px]"}`}
            >
                {/* Header - Make it stick but transparent or matching? */}
                <header className="bg-white border-b border-gray-200 sticky top-0 z-30 mb-4">
