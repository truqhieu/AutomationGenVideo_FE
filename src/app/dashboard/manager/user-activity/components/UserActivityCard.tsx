...
const UserActivityCard = React.memo(({ data, onClick, canClick = true, isActive, timeType }: UserActivityCardProps) => {
    ...
    const statusStyles = {
        warning: {
            card: `border-red-300 border bg-gradient-to-b from-red-50/80 to-rose-50/65 ${canClick ? "hover:from-red-50 hover:to-rose-100/70 transition-all shadow-[0_8px_22px_rgba(239,68,68,0.08)]" : ""}`,
        ...
        completed: {
            card: `border-emerald-300 border bg-gradient-to-b from-emerald-50/80 to-teal-50/65 ${canClick ? "hover:from-emerald-50 hover:to-teal-100/70 transition-all shadow-[0_8px_22px_rgba(16,185,129,0.08)]" : ""}`,
        ...
        exceeded: {
            card: `border-purple-300 border bg-gradient-to-b from-purple-50/80 to-indigo-50/65 ${canClick ? "hover:from-purple-50 hover:to-indigo-100/70 transition-all shadow-[0_8px_22px_rgba(168,85,247,0.1)]" : ""}`,
    ...
    const isReportedOnTime =
        data.reportStatus === "ĐÚNG HẠN" ||
        data.reportStatus === "ĐÃ XONG" ||
        data.reportStatus === "ĐÃ BÁO CÁO ĐỦ" ||
    ...
    return (
        <div
            ...
            className={`relative rounded-xl overflow-hidden transition-all duration-300 ${canClick ? "cursor-pointer hover:scale-[1.01]" : "cursor-default"} ${style.card} ${
                isActive ? "ring-4 ring-blue-500/20 shadow-2xl scale-[1.02] z-10 border-blue-500" : `${style.glow}`
            }`}
        >
            ...
            <div
                ...
                style={{
                    ...
                    backgroundSize:
                        flagBg === "/japan-flag.png" ? "155%" : flagBg === "/thailand-flag.png" ? "110%" : "cover",
            ...
            <div className="absolute inset-0 z-0 pointer-events-none bg-white/52" />
            ...
            <div className="p-3 flex flex-col items-center relative z-10">
                ...
                <div className="absolute top-2 right-2">
                    {statusType === "exceeded" ? (
                        ...
                        <div
                            className={`${!isReportedOnTime ? "bg-red-100 border-red-200" : "bg-gray-100 border-gray-200"} p-2 rounded-xl border`}
                        >
                            <AlertCircle
                                className={`w-4 h-4 ${!isReportedOnTime ? "text-red-500 animate-pulse" : style.icon}`}
                            />
                        </div>
                    )}
                </div>
                ...
                <div className="mt-1 mb-2">
                    <div
                        className={`w-16 h-16 rounded-full border-2 ${style.avatar} p-0.5 transition-all bg-white overflow-hidden shadow-inner`}
                    >
                        <img
                ...
                <div className="text-center mb-3">
                    <h4 className="text-sm font-black text-slate-800 tracking-tight leading-tight mb-1 truncate max-w-[170px]">
                        {data.name}
                    </h4>
                    <div className="flex flex-wrap items-center justify-center gap-1.5 max-w-full">
                        {data.position && (
                            <span
                                className={`text-[10px] font-black px-2 py-0.5 rounded-lg border shadow-xs ${
                                    ["leader", "lead", "quản lý", "tp ", "trưởng"].some((key) =>
                                        data.position?.toLowerCase().includes(key),
                                    )
                        ...
                        <span className="text-[10px] font-black text-blue-700 bg-white px-2 py-0.5 rounded-lg border border-blue-200 shadow-xs flex items-center gap-1">
                            ...
                            {(data.team?.toLowerCase().includes("jp") ||
                                data.team?.toLowerCase().includes("nhật bản")) && (
                                <img
                                    src="/japan-flag.png"
                                    alt="JP"
                                    className="w-3.5 h-2.5 object-contain border border-gray-100"
                                />
                            )}
                ...
                {/* Metrics Grid */}
                <div className="w-full grid grid-cols-1 gap-1.5 mb-3 px-1">
                    <div className="flex items-center justify-between p-1.5 rounded-xl bg-white/90 border border-slate-100 shadow-sm">
                    ...
                    <div
                        className={`flex items-center justify-between p-1.5 rounded-xl border transition-colors ${statusType === "exceeded" ? "bg-purple-50/60 border-purple-100" : "bg-white/90 border-slate-100"}`}
                    >
                        <div className="flex items-center gap-2">
                            <Target
                                className={`w-3.5 h-3.5 ${statusType === "exceeded" ? "text-purple-500" : "text-slate-400"}`}
                            />
                        ...
                        <span
                            className={`text-sm font-black ${statusType === "exceeded" ? "text-purple-700" : "text-slate-900"}`}
                        >
                            {dailyGoal}
                        </span>
                    </div>
                    ...
                    <div
                        className={`flex items-center justify-between p-1.5 rounded-xl border shadow-sm ${statusType === "exceeded" ? "bg-purple-100/80 border-purple-200" : "bg-white/90 border-slate-100"}`}
                    >
                        <div className="flex items-center gap-2">
                ...
                <div className="w-full flex justify-center mb-3">
                    <div
                        className={`px-4 py-1 rounded-xl text-[10px] font-black uppercase tracking-[0.13em] shadow-sm transition-all ${
                            isReportedOnTime
                ...
                <div className="w-full space-y-1.5 mb-3 px-1">
                    <div className="flex justify-between items-center text-[11px] font-black tracking-wider">
                        ...
                        <span className={`${statusType === "exceeded" ? "text-purple-600" : "text-blue-600"}`}>
                ...
                <div className="grid grid-cols-2 w-full gap-2 px-1">
                    <div className="bg-white/90 p-2 rounded-xl border border-blue-100 shadow-sm text-center">
                    ...
                    <div className="bg-white/90 p-2 rounded-xl border border-emerald-100 shadow-sm text-center">
                        <span className="block text-[10px] font-black text-emerald-500 uppercase tracking-widest mb-1">
                            DOANH THU
                        </span>
