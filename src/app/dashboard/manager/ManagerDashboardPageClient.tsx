  ...
  const loadManagerData = async () => {
    ...
    try {
      const [dashRes, chRes] = await Promise.all([
        fetch(`${base}/tracked-channels/manager/dashboard`, { headers }),
        fetch(`${base}/tracked-channels/manager/all-channels`, { headers }),
      ]);
        ...
        <div className="flex items-center justify-between">
          ...
          <p className="text-sm text-slate-600">
            {globalSearchQuery
              ...
              : `Tổng: ${allChannels.length} kênh từ ${userGroups.length} users`}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {["TIKTOK", "INSTAGRAM", "FACEBOOK", "DOUYIN"].map((platform) => {
            ...
            return (
              <div
                key={platform}
                className="bg-white rounded-2xl shadow-lg border-2 border-slate-100 overflow-hidden flex flex-col min-h-[400px]"
              >
                      ...
                      >
                        <div className="flex items-start gap-3 mb-3">
                          <Image
                            src={
                              channel.avatar_url ||
                              `https://ui-avatars.com/api/?name=${encodeURIComponent(channel.display_name)}&background=random`
                            }
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="mt-1.5 inline-flex items-center gap-1.5 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-full px-2.5 py-1">
                              ...
                              <p className="text-xs font-bold text-blue-900">{channel.user.full_name}</p>
                        ...
                        {platform === "TIKTOK" || platform === "DOUYIN" ? (
                          ...
                          <div className="grid grid-cols-2 gap-2 mb-2">
                            ...
                            <div className="bg-white rounded-lg p-2 text-center border border-slate-200">
                              ...
                              <p className="text-sm font-black text-slate-900">
                                {formatNumber(channel.posts_count || channel.total_videos || 0)}
                              </p>
                            </div>
                          </div>
                        ) : platform === "FACEBOOK" ? (
                        ...
                        <div className="pt-2 border-t border-slate-200 text-xs text-center text-slate-400">
                          {channel.last_synced_at
                            ? `Sync: ${new Date(channel.last_synced_at).toLocaleDateString("vi-VN")}`
                            : "Chưa sync"}
                        </div>
                      </div>
                    ))
