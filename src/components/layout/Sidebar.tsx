...
function SmartSidebar({ user, onLogout, isPinned, onTogglePin }: SidebarProps) {
  ...
  // Track if user is actively clicking a link (prevent drawer close during navigation)
  ...
  const handleMouseLeave = useCallback(() => {
    // Don't close drawer if user is navigating
  ...
  // Handle link clicks - keep drawer open during navigation
  ...
  return (
    <>
      ...
      {/* Backdrop - Dims content when drawer is open (not pinned), but doesn't block sidebar */}
      {isDrawerVisible && !isPinned && (
        <div
          className="fixed inset-0 bg-black/10 z-[90]"
          style={{
            transition: prefersReducedMotion ? 'none' : `opacity ${transitionDuration}`,
          }}
