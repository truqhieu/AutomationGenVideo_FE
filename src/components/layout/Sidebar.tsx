...
function SmartSidebar({ user, onLogout, isPinned, onTogglePin }: SidebarProps) {
    ...
    return (
        <Suspense fallback={<div className="w-[40px] bg-[#0f172a]" />}>
            <SidebarContent
                user={user}
                onLogout={onLogout}
                isPinned={isPinned}
...
function SidebarContent({ user, onLogout, isPinned, onTogglePin, allowedMenuIds }: any) {
        ...
        return [
                ...
                menus: [
                    {
                        ...
                        items: [
                            ...
                            {
                                label: "Báo cáo",
                                href: "/dashboard/manager/user-activity?tab=daily_report",
                                icon: FileText,
                            },
                            {
                                label: "Checklist",
            ...
            },
            {
                ...
                menus: [
                    ...
                    {
                        section: "KHÁM PHÁ",
                        items: [{ label: "Tìm kiếm Video (Hub)", href: "/dashboard/search-video", icon: Search }],
                    },
                ],
            },
        ];
    ...
    return (
        <>
            <aside
                ...
                style={{ width: isDrawerVisible ? "320px" : "40px" }}
                onMouseEnter={handleMouseEnter}
                onMouseLeave={handleMouseLeave}
            >
                <div className="w-[40px] h-full bg-[#0f172a] border-r border-slate-800 flex flex-col items-center py-3 gap-3 z-20 relative">
                    <div className="w-8 h-8 rounded-lg overflow-hidden mb-2 cursor-pointer border border-slate-700 shadow-lg relative group">
                        <img
                            src="/logo-vcb.jfif"
                            alt="VCB"
                    ...
                    <div className="flex-1 flex flex-col gap-2.5 w-full px-1.5">
                        {platforms.map((p) => (
                            <Link
                                key={p.id}
                    ...
                    <div className="flex flex-col gap-2.5 w-full px-1.5 mt-auto">
                ...
                >
                    <div className="h-20 flex items-center justify-between px-6 border-b border-slate-800 whitespace-nowrap flex-shrink-0">
                        ...
                        >
                            <Pin
                                className={`w-4 h-4 transition-transform duration-200 ${isPinned ? "fill-current rotate-45" : ""}`}
                            />
                        </button>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto py-6 px-4 whitespace-nowrap custom-scrollbar">
                        {currentPlatform?.menus.map((section, idx) => (
                            <div key={idx} className="mb-8">
                                <h3 className="text-xs font-semibold text-slate-500 mb-4 px-2 tracking-wider">
                                    {section.section}
                                </h3>
                                <div className="space-y-1">
                                    {section.items.map((item: any) => {
                                        ...
                                        const itemTab =
                                            item.href.indexOf("tab=") !== -1 ? item.href.split("tab=")[1] : null;
                                        ...
                                        return (
                                            ...
                                            >
                                                <item.icon
                                                    className={`w-4 h-4 transition-colors duration-150 ${isActive ? "text-blue-500" : "text-slate-500"}`}
                                                />
                                                {item.label}
                                            </Link>
                                        );
